...
export function DemoControls({ 
  ...
  const handleRunBatch = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/v1/pipeline/run-batch`, {
        ...
        body: JSON.stringify({ n_events: 50, policy: 'bandit', random_seed: Date.now() % 100000 }),
      ...
      onRunCompleted?.();
      setRunMessage(`Simulation completed: ${result.n_events ?? 50} events processed.`);
    } catch (e) {
  ...
  return (
    <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      >
        {isRunning ? "Starting..." : "Run Batch Simulation (50)"}
      </button>
