...
class FakeStore:
    ...
    def get_stats(self, bucket, arm):
        return self.data[arm][0], self.data[arm][1]
        
    def increment_alpha(self, bucket, arm, amount):
        self.data[arm][0] += amount
        
    def increment_beta(self, bucket, arm, amount):
...
def test_gate_blocked_penalty_only_touches_beta(bandit_with_fake_store, fake_store):
    alpha_before, beta_before = fake_store.get_stats("bucket", "retry_immediate")
    bandit_with_fake_store.update("bucket", "retry_immediate", reward=-0.1)
    alpha_after, beta_after = fake_store.get_stats("bucket", "retry_immediate")
